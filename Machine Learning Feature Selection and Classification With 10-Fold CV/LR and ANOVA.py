...
# 1. Reduce the number of features to 2000 using LinearSVC if possible
...
# 2. Further feature selection using ANOVA and Logistic Regression-RFE

# ANOVA
num_features = min(2000, X_lsvc.shape[1])
anova_selector = SelectKBest(f_classif, k=num_features).fit(X_lsvc, y)
X_anova = anova_selector.transform(X_lsvc)
selected_anova_features = selected_lsvc_features[anova_selector.get_support()]
...
selected_rfe_features = selected_anova_features[rfe_selector.get_support()]
