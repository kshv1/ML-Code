...
# 2. Further feature selection using Mutual Information and Logistic Regression-RFE

# Mutual Information
num_features = min(2000, X_lsvc.shape[1])
mutual_info_selector = SelectKBest(mutual_info_classif, k=num_features).fit(X_lsvc, y)
X_mi = mutual_info_selector.transform(X_lsvc)
selected_mi_features = selected_lsvc_features[mutual_info_selector.get_support()]
print(f"Features after Mutual Information: {X_mi.shape[1]}")
...
X_rfe = rfe_selector.fit_transform(X_mi, y)
selected_rfe_features = selected_mi_features[rfe_selector.get_support()]
